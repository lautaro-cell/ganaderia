...
public class LivestockServiceImplementation : LivestockService.LivestockServiceBase
{
    ...
    {
        var id = await _userService.InviteUserAsync(new WebApplication1.Application.DTOs.InviteUserRequest(request.Email, request.Name, request.RoleName, Guid.Parse(request.TenantId)));
        return new ActionResponse { Success = true, ObjectId = id.ToString() };
    }
